...
In @sec-treats, it was mentioned that one
...
point $i$. But in @sec-curric, concern was raised as to the degree of
statistical sophistication needed. Let's take a look.

```{r}
lmOut <- lm(wageinc ~ .,svc)
coef(lmOut)
xpt <- svc[1,-9]
xptv <- unlist(xpt)
regXpt <- predict(lmOut,xpt)
contrib <- coef(lmOut)['gender.female']
a <- c(1,rep(0,9),1)
b <- c(1,xptv)
cvm <- vcov(lmOut)
t(a) %*% cvm %*% b

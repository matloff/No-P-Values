...
5. Non-NHST tools should be developed in that direction, some presented
   here.

6. Such tools must be used to inform the analyst but not 
...
but also simply unanswerable.
   ...
   * E.g. physics, $H_0:$ gravity waves do not exist.
      ...
      * Yes, the existence is either true or false, but our imperfect
        measurements make $H_0$ false; we large enough $n$, we will
        reject.
...
There are important causal issues here and beloew, but this aspect is beyond the
...
impllied) from Dylan Atmbruster and Boyu Wang
...
As discussed, the classic formulas for the
...
Let's look at a linear model example, again using the **svcensus** data.
...
repOut <-
   replicate(nreps,
      coef(qeLin(svcensus,'wageinc',holdout=100))['gendermale'])
mean(repOut)
sd(repOut) 
```
...
But in the above code, we found a CI for $\beta_{\textrm{gender}}$ -- a
quantity that has no counterpart in general regression functions. What
might be done?

One possibility would be to consider $\beta_{\textrm{gender}}$ to
represent an *average treatment effect*, essentially

$$
E(\textrm{wage income} | \tilde{X},\textrm{gender = male}) -
E(\textrm{wage income} | \tilde{X},\textrm{gender = female})
$$

where $\tilde{X}$ denotes the predictors other than gender.
One could use k-Nearest Neighbors or kernel regression to
estimate the two gender-specific real functions, then compare to
our estimate of $\beta_{\textrm{gender}}$ to obtain our distance value.
...
accept such analyses. As long as (possibly anonymized) data, code and random number seed are

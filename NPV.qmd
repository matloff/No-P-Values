...
2. Here we probe more deeply than typical criticisms of NHST in the
   literature, starting with this core theme that all $H_0$s are false.

3. Null Hypothesis Significance Testing (NHST) is thus an absurdity.
...
* A CI for the contribution of an interaction term to an overall 
  estimate of $E(Y|X=t)$ is straightforward. 

  $$

  -2.876555 \pm  0.460281 = (-3.336836, -2.416274)
...
* However, the more relevant quantity might be the net effect of being
  ...
  still straightforward, if the analyst's statistical coursework had
  included dealing with covariance matrices (available here via the
  **vcov** function). 
...
combination of the components of a vector $\widehat{\theta}$, knowing
its estimated covariance matrix.  I would surmise that even most
Master's programs in Statistics do not cover this, and really the
concept is not beyond the grasp of students methodology courses 
that do not require linear algebra, say a course in regression for
non-Statistics majors. If the course uses the R **lm** function, there
is no reason students can't be taught the associated **vcov** method.

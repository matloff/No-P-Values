...
*All models are wrong, but some are useful* -- prominant 
...
* But if the goal is measurement of treatment effects, we may be
  acquiring distorted results if the model discrepancy is large.
...
* Even though our model is (always) wrong, we are still estimating
  *something*. If we fit a linear regression model $E(Y|X=t) = \beta'X$
  ...
  population regression function $\mu(t) = E(Y|X=t)$. $\beta$ is then
...
* However, our estimation procedure for that closest model should not
  ...
  For instance:
     ...
     * Normally, in the linear miodel, we estimate the conditional
     ...
     * In the case of MLEs, the standard errors come from the
       *information matrix*, based on a derivation that assumes the
       model is correct. Thus standard errors based on the classic
       formmula are incorrect.
...
* But once again, "all models are wrong," so in addition to calculating
...
* However, most have limitations from our point of view here. For
  instance, the procedure should produce standard errors not only for
  $\theta$, the population quantity of interest, but also for the
  distance from the asymptotic model fit and the true population
  process. Most important, the procedure must not assume correctness of
  the model.
...
That first barrier again illustrates the aboe point regarding
...
# replace gender by dummy variable for female
...
* We have a model $\cal{M}$, again necessarily incorrect, for the
  ...
  regression function, but for simplicity will refer just to the dstr.)
...
* Optionally, we may want to also estimate the population "distance"
  between  $F_W$ model and $F_{pop}$, including a standard error for
  that estimate.
...
to estimate $\lambda$ accordingly. We also want to assess the distance
between our model and the true population state of affairs. Denote our
data by $X_1,...,X_n$.

The MLE of $\lambda$ is $\widehat{\lambda} = 1/\bar{X}$, the reciprocal
of the sample mean.  As discussed, the classic formulas for the
standard error of $\widehat{\lambda}$ are invalid here. But the solution
is simple here: $\bar{X}$ is asymptotically normal from the Central
Limit Theorem, with standard error computable from the data. Thus we can
find a CI for E(X), then invert the endpoints to obtain an interval for
$\lambda$.
...
simplicity, let's look at just one point, say $v$. We thus wish to find
...
according to whethe $X_j \leq v$.

# Making a Decision, III

As noted earlier, proper analysis is easier, and probably less
psychologically satisfying than NHST. In this section, there is also the
issue of whether a typical scientific researcher even has the backgrund
...
This then is a good example of the need for mathematical statisticians t
develop non-NHST tools, say along the above lines.
...
accept such analyses. As long as data, code and random number seed are
made available, hopefully this could be worked out.

...
::: {.callout-important title="'Significant' Has No Meaning"}
...
* It is very telling that in a special 2019 issue of the *American
  Statistician* devoted to NHST problems, some authors proposed
  elaborate replacements for NHST. The psychological urge for an
  automatic decision-making procedure is clearly very strong. But I
  would argue that it is counterproductive, not really addressing the
  problem at hand.
...
* A CI for the contribution of an interaction term to an overall 
  estimate of $E(Y|X=t)$ is straightforward. The interaction term for GPA
  and being Black is given in the output, and we can add and subtract
  1.96 times the standard error to obtain a CI for a 1-point increase in
  GPA for Black students, for example.

* A CI for the *proportional* contribution to $E(Y|X=t)$ can be obtained via
  the Delta Method, or by the bootstrap.
